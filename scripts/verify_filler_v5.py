...
# ----- fakes ($0; no network) -----
class _Speech:
    def __init__(self, delay): self._d = delay
...
class FakeOAI:
    def __init__(self, content_synth_delay=0.05):
        self.audio = types.SimpleNamespace(speech=_Speech(content_synth_delay))
...
def run_case(name, route_fn, gate_fn, ttft, topic_synth_delay=1.2, cache_hit=False,
    ...
    job = voice_job.start_job("Q?", "TEST", 0.5, FakeOAI(), set(), None, "onyx", 0,
                              decks, seq_state, _route_fn=rf, _gate_fn=gate_fn,
                              _retrieve_fn=stub_retrieve(), _compose_fn=stub_compose(ttft))
    while not job["done"]:
        time.sleep(0.02)
    chain = "-".join(job["chain"])
    # annotate silence between the last filler and content
    disp = chain
    if job.get("silence_gap_ms"):
        disp = chain.replace("-C", "-[silence]-C", 1)
...
for c in CASES:
    ...
    results.append(r)
    extra = []
    if r["theme_used"]: extra.append(f"theme={r['theme_used']}")
    if r["topic_used"]: extra.append(f"topic={r['topic_used']} margin={r['topic_margin']} cache_hit={r['cache_hit']}")
    ...
    print(f"  {r['case']:28s} -> {r['chain_annot']:16s} conf={r['route_confidence']} "
...
EXPECT = {"1 confident+separated/slow": "T-P-C", "2 confident+near-tie": "T-C",
          ...
          "8 late route (>300ms)": "N-C"}
print("\n=== assertions ===")
ok = True
for r in results:
    exp = EXPECT[r["case"]]
    good = r["chain"] == exp and not r["error"]
    ...
    print(f"  {'PASS' if good else 'FAIL'}  {r['case']:28s} chain={r['chain']} expect={exp}"
          + ("  (+silence_gap>0)" if r["case"] == "7 long transient" else ""))
...
out = {"cases": results, "all_pass": ok, "clip_stats": man["duration_stats"],
       "over_cap": man["over_cap"], "grammar_drops": len(drops)}
